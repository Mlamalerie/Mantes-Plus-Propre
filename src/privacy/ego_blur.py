...
def blur_detections_in_image(
    ...
    if face_detector is not None:
        print("Performing face detections...")
        detections.extend(
            get_detections(
                face_detector,
                image_tensor,
    ...
    if lp_detector is not None:
        print("Performing license plate detections...")
        detections.extend(
            get_detections(
                lp_detector,
                image_tensor_copy,
    ...
    image = apply_blur_to_detections(
        image,
        detections,
        scale_factor_detections,
    )
...
def blur_detections_in_video(
    ...
    frames = video_reader_clip.iter_frames()
    for frame in tqdm(frames, total=video_reader_clip.reader.nframes):
...
def load_face_detector(model_path : str):
...
if __name__ == "__main__":
    ...
    duration = mid_end_time - start_time
    duration_min, duration_sec = divmod(duration, 60)
    print(f"Loading models took {duration_min:.0f} minutes and {duration_sec:.0f} seconds")

    if args.input_image_path is not None:

        image = blur_detections_in_image(
    ...
    duration_min, duration_sec = divmod(duration, 60)
    print(f"Demo took {duration_min:.0f} minutes and {duration_sec:.0f} seconds")
    print(f"Demo finished. Output saved at {args.output_image_path if args.input_image_path is not None else args.output_video_path}")
